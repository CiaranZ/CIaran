...
#include <ocs2_robotic_tools/common/RotationDerivativesTransforms.h>
  ...
  // Eigen::Matrix<scalar_t, 18, 1> x_hat_; p , p_dot,pw,pw_dot,pcp1,pcp2,pcp3,pcp4   6+3*4   6+
...
vector_t KalmanFilterEstimate::update(const ros::Time& time, const ros::Duration& period)
{
  ...
  q_pino.setZero();
  // q_pino.segment<3>(0) = rbd_state_.segment<3>(3);
  ...
  v_pino.setZero();
  // v_pino.head<3>() = rbd_state_.segment<3>(generalized_coordinates_num_ + 3);
  ...
  current_state_temp.tail(actuated_dof_num) = rbd_state_.segment(6, actuated_dof_num);
  

  const auto ee_pos = pinocchio_ee_kine_.getPosition(current_state_temp);
  const auto ee_vel = pinocchio_ee_kine_.getVelocity(vector_t(), vector_t());

  scalar_t foot_radius = 0.05;
