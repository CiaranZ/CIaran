...
void LeggedInterface::setupOptimalControlProblem(const std::string& taskFile, const std::string& urdfFile,
                                                 const std::string& referenceFile, bool verbose)
{
  setupModel(taskFile, urdfFile, referenceFile, verbose);        //pinocchioInterfacePtr_在这个里面初始化

  std::cerr << "\n#### Modes nq: " << pinocchioInterfacePtr_->getModel().nq;                  //18  12+3+3
  std::cerr << "\n#### Modes nv: " << pinocchioInterfacePtr_->getModel().nv;                  //18  16+3+3
  // std::cerr << "\n#### Modes njoints: " << pinocchioInterfacePtr_->getModel().njoints;        
  std::cerr << "\n#### Modes actuatedDofNum: " << centroidalModelInfo_.actuatedDofNum;        //12
  std::cerr << "\n#### Modes stateDim: " << centroidalModelInfo_.stateDim;                    //24 16+3+3+3+3
  std::cerr << "\n#### Modes inputDim: " << centroidalModelInfo_.generalizedCoordinatesNum;                    //24 16
  ...
  referenceManagerPtr_ = std::make_shared<SwitchedModelReferenceManager>(loadGaitSchedule(referenceFile, verbose),
                                                                         std::move(swing_trajectory_planner));    //这里只规划抬角高度
  ...
  dynamics_ptr.reset(
      new LeggedRobotDynamicsAD(*pinocchioInterfacePtr_, centroidalModelInfo_, "dynamics", modelSettings_));    //动力学模型约束
  problemPtr_->dynamicsPtr = std::move(dynamics_ptr);

  // Cost terms
  problemPtr_->costPtr->add("baseTrackingCost", getBaseTrackingCost(taskFile, centroidalModelInfo_, verbose));//是所有状态的轨迹跟踪，因为Q和R的维度可以看出，看看能否和MIT一样简化
  ...
  std::cerr << "\n#### check warning#### \n" ;
  for (size_t i = 0; i < centroidalModelInfo_.numThreeDofContacts; i++)//这里就是4个接触点
  {
    ...
    problemPtr_->equalityConstraintPtr->add(foot_name + "_zeroForce",
                                            std::unique_ptr<StateInputConstraint>(new ZeroForceConstraint(*referenceManagerPtr_, i, centroidalModelInfo_)));//open contacts 0力
    problemPtr_->equalityConstraintPtr->add(foot_name + "_zeroVelocity",
                                            getZeroVelocityConstraint(*ee_kinematics_ptr, i));            //  这里就是末端的速度和位置的约束
    problemPtr_->equalityConstraintPtr->add(foot_name + "_normalVelocity",
                                            std::unique_ptr<StateInputConstraint>(new NormalVelocityConstraintCppAd(
                                                *referenceManagerPtr_, *ee_kinematics_ptr, i)));          //  swing legs需要跟踪一个沿着法线的速度
  }
...
{
  auto ee_zero_vel_con_config = [](scalar_t positionErrorGain) {   //这玩意是0
    EndEffectorLinearConstraint::Config config;
    config.b.setZero(3);
    config.Av.setIdentity(3, 3);
    config.Av(0,0)= 0; //x方向不受约束
    // config.Av(1,1)= 0; //y方向不受约束
    // config.Av(2,2)= 2; //z方向不受约束
